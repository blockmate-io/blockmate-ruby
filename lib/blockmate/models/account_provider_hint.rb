  ...
  class AccountProviderHint
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Blockmate::AccountProviderHint` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Blockmate::AccountProviderHint`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      account_type_validator = EnumAttributeValidator.new('String', ["crypto_wallet", "crypto_exchange", "crypto_nft"])
    ...
    def hash
      [account_name, account_type, account_url, description, url, intro, fields, oauth, icon].hash
    end
